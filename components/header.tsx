  ...
  return (
    <header className="sticky top-0 z-40 border-b border-warm-200 bg-warm-50/90 backdrop-blur-sm">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        ...
        <button
          type="button"
          className="flex items-center justify-center rounded-full border-2 border-brand-red p-1.5 md:hidden"
      ...
      {isOpen && (
        <nav
          id="mobile-menu"
          className="border-t border-warm-200 md:hidden"
          ...
          <ul className="divide-y divide-warm-200">
            {navLinks.map(({ href, label }) => (
              <li key={href}>
                <Link
                  href={href}
                  className={`block px-4 py-3 text-center text-sm font-medium transition-colors hover:bg-warm-100 ${
                    pathname === href
                      ? "text-brand-red"
                      : "text-warm-800"
                  }`}
